...
def debug_logs_get():
    """Returns the Bassline debug log as a plaintext HTTP response.
...
def restart_post():
    ...
    and `error` properties in the response. This is needed for when Bassline
    updates from a version before our migration to using conventional HTTP
    status codes. Issue: https://github.com/bass-line/bassline/issues/506
...
def update_put():
    """Initiates job to update Bassline to the latest version available.

    This endpoint asynchronously starts a job to update Bassline to the latest
...
def version_get():
    """Retrieves the current installed version of Bassline.
...
def latest_release_get():
    """Retrieves the latest version of Bassline.
