...
UPDATE_SCRIPT_PATH = '/opt/bassline-privileged/update'


def start_async():
    """Launches the update service asynchronously.

    Launches the bassline-update systemd service in the background. If the
    ...
    subprocess.Popen(
        ('sudo', '/usr/sbin/service', 'bassline-updater', 'start'))
