"""Manages a Bassline update settings file.

Bassline currently manages most settings through Ansible. When Bassline
...
allows Bassline code to modify it cleanly.

Typical usage example:

    settings = update_settings.load()
    settings.bassline_repo_branch = '2.1.5'
...
class Settings:
    ...
    # Note: bassline_repo_branch is confusingly named. It should really be
    # bassline_repo_version, but this class just reflects the names in the
    # Bassline Ansible role.
    @property
    def bassline_repo_branch(self):
        return self._data['bassline_repo_branch']

    @bassline_repo_branch.setter
    def bassline_repo_branch(self, value):
        """Sets the value of bassline_repo_branch in update settings.

        Args:
            value: A string value of a branch or tag name like '2.1.0' or
                'virtual-storage'.
        """
        self._data['bassline_repo_branch'] = value
...
def load():
    """Retrieves the current Bassline update settings
